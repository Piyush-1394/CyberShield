...
export const useRecommendations = (budget) => useQuery({ queryKey: ['invest-rec', budget], queryFn: () => api.get('/api/investment/recommendations', { params: budget ? { budget } : {} }).then((r) => r.data) })
export const useProjection = (budget) => useQuery({ queryKey: ['invest-proj', budget], queryFn: () => api.get('/api/investment/projection', { params: budget ? { budget } : {} }).then((r) => r.data) })
